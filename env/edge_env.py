...
class EdgeEnv():
    def __init__(self, general_params, writer = None):
        ...
        self.old_comp_queue_delay = [0 for _ in range(self.edge_queue_num)]
    ...
    def compute(self, device_sched_tasks, e_id, t_id, visualize=False):
        ...
        total_comp_used_time_this_epi = [min(gap, self.edge_queue_time_ql[i]) for i in range(self.edge_queue_num)]
        comp_dlys = [min(gap, self.edge_queue_time_ql[i]) for i in range(self.edge_queue_num)]
        self.delta_num += 1
        for device_type in range(self.edge_queue_num):
            ...
            for i,task in enumerate(device_type_sched_tasks[device_type]):
                if task.trans_time == 0:
                    ...
                    task.e_queue_dly = self.old_comp_queue_delay[device_type] + new_comp_queue_delay
                    task.e_proc_dly = task.offl_dz * task.comp_dens / alloc_edge_freq[device_type]
                    # print(f"[DEBUG] the task.offl_dz in device {task.device_id} is {task.offl_dz}")
                    # print(f"[DEBUG] the task.comp_dens in device {task.device_id} is {task.comp_dens}")
                    # print(f"[DEBUG] the alloc_edge_freq in device {task.device_id} is {alloc_edge_freq[device_type]}")
                    ...
                    # print(f"[DEBUG] the old_comp_queue_delay in device {task.device_id} is {self.old_comp_queue_delay[device_type]}")
                    # print(f"[DEBUG] the new_comp_queue_delay in device {task.device_id} is {new_comp_queue_delay}")
                    # print(f"[DEBUG] the e_queue_dly in device {task.device_id} is {task.e_queue_dly}")
                    # print(f"[DEBUG] the trans_time in device {task.device_id} is {task.trans_time}")
                    # print(f"[DEBUG] the e_proc_dly in device {task.device_id} is {task.e_proc_dly}")
                    # print(f"[DEBUG] the edge comp_dly in device {task.device_id} is {task.e_comp_dly}")
                    comp_dlys[device_type] = max(comp_dlys[device_type], task.trans_time) + task.e_proc_dly
                    new_comp_queue_delay += task.e_proc_dly
                    self.total_comp_amount[device_type] += task.offl_dz * task.comp_dens
        
                # avg_edge_time: mean over last statSlotNum slots
                self.old_comp_times[device_type].append(task.e_proc_dly)
            ...
            if t_id % self.gen_task_cycle == 0:
                self.old_comp_queue_delay[device_type] = max(self.old_comp_queue_delay[device_type] + new_comp_queue_delay - gap, 0)
                ...
                self.new_edge_ql_change[device_type] = edge_act_queue_growth_rate * (self.act_backlog - total_comp_used_time_this_epi[device_type])
                self.edge_queue_time_ql[device_type] = max(self.edge_queue_time_ql[device_type] + self.new_edge_ql_change[device_type], 0)

                # print(f"[DEBUG] The edge_queue", device_type, "'s edge_queue_time_ql: ", self.edge_queue_time_ql[device_type])
                # print(f"[DEBUG] The edge_queue", device_type, "'s edge_act_queue_growth_rate: ", edge_act_queue_growth_rate)
                ...
                # print(f"[DEBUG] The edge_queue", device_type, "'s new_edge_ql_change: ", self.new_edge_ql_change[device_type])
                ...
                if self.avg_edge_time[device_type] > EPS:
                    self.new_vir_edge_ql_change[device_type] = edge_vir_queue_growth_rate*(self.edge_queue_time_ql[device_type]/self.avg_edge_time[device_type]*self.delta*self.gen_task_cycle/self.device_num_per_type[device_type] - self.edge_dly_adj_val[device_type])
